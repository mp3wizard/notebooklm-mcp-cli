...
def test_is_snap_browser_symlink(tmp_path):
    ...
    with patch.object(
        Path, "resolve", return_value=Path("/snap/chromium/current/usr/bin/chromium")
    ):
...
def test_get_snap_common_dir_from_resolved_parts():
    ...
    with (
        patch.object(Path, "resolve", return_value=Path("/snap/chromium/current/usr/bin/chromium")),
...
def test_get_snap_common_dir_fallback(tmp_path):
    ...
    with (
        ...
        patch.object(Path, "resolve", return_value=Path("/snap/bin/browser")),
    ):
